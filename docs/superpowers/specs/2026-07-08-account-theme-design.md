...
意味を持つ状態色であり、ブランドカラーではないため、テーマに関わらず統一する：
- アラート（`.alert-success` / `.alert-danger` / `.alert-warning` / `.alert-info`）
- ステータスバッジ（`.badge-pending` / `.badge-queued` / `.badge-posted` / `.badge-rejected` / `.badge-failed`）
- YouTube収集ボタン（YouTube公式のブランドカラー、サイトテーマと無関係）
- テーブルhover・フォームフォーカスリングなど、微細で目立たない箇所

これらは `#1B5E20` や `rgba(74,158,107,...)` を使っていても、意図的に変数化・上書き対象から外す
（`.alert-danger` の `#1B5E20` は文字色として使われているのみで、上記「個別上書き対象」には含めない）。
