...
**既知の機能後退（意図的に許容）:** 実装後の最終レビューで判明した点として、削除された
ページ内タブ（`pending`/`queue`/`schedule` の旧アカウント選択タブ）はページのコンテンツ領域に
あったため、実はモバイルでも表示・操作可能だった。新しいサイドバードロップダウンはデスクトップ
専用（`d-none d-md-block`）のため、本機能のリリースにより**モバイルではアカウントを切り替える
手段が一切なくなる**。これは意図的に許容し、今回のスコープでは対応しない。モバイル対応が必要に
なった場合は別タスクとして着手する。
